"""Rscript 执行引擎：定位 R、序列化参数、运行 mscripts/ 下的微生物组 R 模板并解析结果。

设计要点（与 rgraph-mcp 一致的架构，便于统一维护）
--------------------------------------------------------
* **忠实 R 出图**：不在 Python 里重画，而是驱动一套清洗过的参数化 R 脚本（vegan/DESeq2/
  edgeR/igraph/Hmisc/randomForest/ggtree/ggplot2 等），产出出版级 png/pdf。
* **零额外 R 依赖传参**：参数以「R 源文件」形式落盘（`params <- list(...)`），R 端 `source()`
  即可，不依赖 jsonlite 等包。
* **优雅降级**：定位不到 Rscript → 返回可直接手动运行的脚本与命令；R 端缺包 → 返回缺失包名与
  安装建议（区分 CRAN/Bioconductor），而不是抛出晦涩的 R 报错。
* **可复现**：以 `--no-init-file --no-site-file` 运行，屏蔽用户 .Rprofile/Rprofile.site
  的自动加载横幅，避免污染 stdout 标记解析。
"""
...
def find_rscript() -> str | None:
    """按优先级定位 Rscript：环境变量 MICROBE_RSCRIPT → PATH → 常见安装目录/注册表。"""
    env = os.environ.get("MICROBE_RSCRIPT")
...
def mscripts_dir() -> Path:
    """定位 mscripts 目录（源码布局为包的同级；wheel 布局为包内）。"""
    here = Path(__file__).resolve().parent
    for cand in (here.parent / "mscripts", here / "mscripts"):
        if cand.is_dir():
            return cand
    return here.parent / "mscripts"
...
def build_command(script_name: str, params_file: Path, rscript: str | None = None) -> list[str] | None:
    ...
    if not rscript:
        return None
    script = mscripts_dir() / script_name
    common = mscripts_dir() / "_common.R"
...
def run_script(
    ...
    *,
    ...
    返回字段：
      ...
      outputs: 产物文件绝对路径列表（R 端通过 `MICROBE_OUTPUT:` 标记回传）
    ...
    param_dir = Path(outdir) if outdir else Path(tempfile.mkdtemp(prefix="microbe_"))
    params_file = param_dir / f".microbe_params_{script_name.replace('.R', '')}.R"
    ...
    if cmd is None:
        ...
        return {
            "status": "r_not_found",
            "message": ("未找到 Rscript。请安装 R 或设置环境变量 MICROBE_RSCRIPT 指向 Rscript(.exe)。"
                        "已生成可手动运行的脚本与参数文件。"),
            "script": str(script_path),
    ...
    for line in combined.splitlines():
        line = line.strip()
        if line.startswith("MICROBE_OUTPUT:"):
            outputs.append(line[len("MICROBE_OUTPUT:"):].strip())
        elif line.startswith("MICROBE_MISSING_PACKAGES:"):
            pk = line[len("MICROBE_MISSING_PACKAGES:"):].strip()
            missing = [x.strip() for x in pk.replace(";", ",").split(",") if x.strip()]
        elif line.startswith("MICROBE_METRIC:"):
            metrics.append(line[len("MICROBE_METRIC:"):].strip())
...
_BIOC = {
    "DESeq2", "edgeR", "limma", "phyloseq", "microbiome", "ALDEx2", "metagenomeSeq",
    "ANCOMBC", "ggtree", "treeio", "ComplexHeatmap", "DirichletMultinomial",
    "BiocManager", "SummarizedExperiment", "Biobase",
}
